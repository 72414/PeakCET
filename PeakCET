...
class MForm(tk.Frame):
    ...
    def initComponent(self, master):
        ...
        img_1 = cv2.imread("1.jpg") #Set Cover Picture
        img_1 = cv2.resize(img_1, (1300, 1100))
        cv2.imwrite(r"...\1.jpg", img_1)
        img_open = imim.open("1.jpg")
        img_png = ImageTk.PhotoImage(img_open)
        self.label_img = tk.Label(self.frm_right, image=img_png)  # label
        self.label_img.image = img_png
        self.label_img.grid()
        # self.frm_control = ttk.Frame(self.frm_right, relief=tk.RAISED)
    ...
    def initPlayList(self):
        ...
        tr_root = tree.insert("", 0, None, open=True, text='List')
    ...
    def submit(self):
        ...
        global imglabel
        nIndex = nIndex + 1
        print("Observe individual contour:", PngList[nIndex - 1])
    ...
    def contours_demo(self,image, ccc, mat_name):
        ...
        for i, contour in enumerate(contours):
            ...
            cv2.imwrite(".../png" + "/" + str(i) + ".jpg", image)
        for j in range(row):
    ...
    def trans_normalize(self,R,y_long,y_drift):
        ...
        for q in range(y):
            if q < y_drift:
                for j in range(x):
                    CA[q][j] = F[q + (y-y_drift)][j]
            else:
                for j in range(x):
                    CA[q][j] = F[q - y_drift][j]
        # for q in range(y):
        #     for j in range(x):
        #         CA[q][j]=float(CA[q][j])
        minA = CA.min()
    ...
    def ap_second(self):
        ...
        for jk in range(len(con_num)):
            ...
            for i in range(len(col)):
                ...
                max_xy.append([])
                max_index.append([])  #Find the maximum values of all columns within the contour
            for i in range(len(col)):
                if max_value[i] != []:
        ...
        # ========================
        print("Peak number after AP clustering", len(m_re))
    ...
    def func1_2(self):
        ...
        print("The file was successfully imported and the original image was generated. The contour detection is complete.")
        print("Peak number after contour detection",len(re1))
    ...
    def func2(self):
        ...
        self.label_img.config(image=img_png2)
        self.label_img.image = img_png2
        self.label_img.grid()
    ...
    def func3(self):
        ...
        self.label_img.config(image=img_png3)
        self.label_img.image = img_png3
        self.label_img.grid()
    def func4(self):
        img = Image.open(r'...\png\0.jpg')
        # Open the first contour image
        img_one = ImageTk.PhotoImage(img)
        self.label_img.config(image=img_one)
        self.label_img.image = img_one
        self.label_img.grid()
