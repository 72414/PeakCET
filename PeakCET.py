...
class MForm(tk.Frame):
    ...
    def initComponent(self, master):
        ...
        self.frm_right = ttk.Frame(self.panewin, relief=tk.SUNKEN, style='www.TFrame')  # 右侧Frame帧用于放置视频区域和控制按钮
        ...
        self.panewin.add(self.frm_right, weight=5)  # 将右侧Frame帧添加到推拉窗控件,右侧权重10
        #第三个
        self.frm_right2 = ttk.Frame(self.panewin, relief=tk.SUNKEN,width=300)
        ...
        img_1 = cv2.imread(r"C:\Users\yy.Xinyue\PycharmProjects\ProjectEdge\1.jpg")
        img_1 = cv2.resize(img_1, (1300, 1100))
        cv2.imwrite(r"C:\Users\yy.Xinyue\PycharmProjects\ProjectEdge\1.jpg", img_1)
        img_open = imim.open(r"C:\Users\yy.Xinyue\PycharmProjects\ProjectEdge\1.jpg")
        img_png = ImageTk.PhotoImage(img_open)
        self.label_img = tk.Label(self.frm_right, image=img_png)  # 定义label
    ...
    def initMenu(self, master):
        ...
        mbar.add_cascade(label='Input', menu=fmenu, font=('Times', 20, 'bold'))  # 添加子菜单
        ...
        mbar.add_cascade(label='Theme', menu=fmenu2, font=('Times', 20, 'bold'))  # 添加子菜单
        ...
        fmenu2.add_separator()  # 添加分割线
        fmenu2.add_command(label="Quit", command=root.quit)
        # fmenu3 = tk.Menu(mbar, tearoff=False)  # 在顶级菜单下创建菜单项
        # mbar.add_cascade(label='Instruction', menu=fmenu3, font=('Times', 20, 'bold'))  # 添加子菜单
        # fmenu3.add_command(label="About the program of PeakCET", command=self.information)
        # fmenu3.add_separator()  # 添加分割线
        # fmenu3.add_command(label="Quit", command=root.quit)
    ...
    def initPlayList(self):
        ...
        tree.column('#0', width=300)  # 设置图标列的宽度，视图的宽度由所有列的宽决定
        ...
        node1 = tree.insert(tr_root, 0, None, open=True, text='PeakCET results')  # 根节点下添加一级节点
        node11 = tree.insert(node1, 0, None, text='Contours')  # 添加二级节点
    ...
    def initCtrl(self):
        '''初始化按钮'''
        lab_1 = tk.Button(self.frm_right2, text="Previous",command=self.pre)  # 标签
        lab_2= tk.Button(self.frm_right2, text="Next",command=self.submit)  # 标签显示视频长度
        lab_1.place(x=20, y=550)
        lab_2.place(x=20, y=620)
        lab_3 = tk.Button(self.frm_right2, text="First",command=self.home)  # 标签
   ...
   #首页
    ...
    def ap_second(self):
        ...
        global window2
        window2 = tk.Tk()
        window2.title('The result of AP clustering')
        ...
        imgnew = cv2.resize(imgnew, (1300, 800))
    ...
    def func1(self):
        ...
        b1 = tk.Button(window1, text="Enter",command=self.func1_2)  
    ...
    def func2(self):
        ...
        show_im1 = cv2.resize(show_im1, (1300, 1100))
    ...
    def func3(self):
        ...
        show_im2 = cv2.resize(show_im2, (1300, 1100))
